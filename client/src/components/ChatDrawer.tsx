...
export default function ChatDrawer({ open, onClose }: ChatDrawerProps) {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-50 w-95 max-w-[calc(100vw-2rem)] h-130 bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
            {msg.sender === "bot" && (
              <div className="w-6 h-6 bg-maroon rounded-full flex items-center justify-center shrink-0">
